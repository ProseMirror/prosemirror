...
test("preserve", pm => {
  ...
  pm.apply(pm.tr.insertText(P(0, 0), "A").insertText(P(0, 1), "B"))
  cmpStr(range.from, P(0, 3))
  cmpStr(range.to, P(0, 6))
  pm.apply(pm.tr.delete(P(0, 4), P(0, 5)))
  cmpStr(range.from, P(0, 3))
...
test("leftInclusive", pm => {
  ...
  pm.apply(pm.tr.insertText(P(0, 1), "X"))
...
test("rightInclusive", pm => {
  ...
  pm.apply(pm.tr.insertText(P(0, 2), "X"))
...
test("deleted", pm => {
  ...
  pm.apply(pm.tr.insertText(P(0, 1), "A"))
  cmp(cleared, false)
  pm.apply(pm.tr.delete(P(0, 2), P(0, 4)))
  cmp(cleared, true)
  cmp(range.from, null)
})
...
test("stay_when_empty", pm => {
  ...
  range.on("removed", () => cleared = true)
  pm.apply(pm.tr.delete(P(0, 0), P(0, 4)))
  cmp(cleared, false)
